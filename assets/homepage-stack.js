...
(function () {
  ...
  function setupStack(cards, pusher) {
    ...
    // Pre-allocate arrays to guarantee contiguous O(1) memory mapping
    ...
    // OPTIMIZATION 1: Extract heavy computed styles OUT of the scroll loop
    function cacheStaticMeasurements() {
      for (var i = 0; i < cards.length; i++) {
    ...
    function calculateCardTranslates(pusherTop, headerBottom) {
      ...
      var thirdNaturalTop = rects[THIRD_CARD_INDEX].top + appliedTranslates[THIRD_CARD_INDEX];
      var maxUpToHeader = Math.max(0, thirdNaturalTop - headerBottom - GAP_BELOW_HEADER);
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        if (Math.abs(previousTranslate - nextTranslate) > 0.1) {
          if (nextTranslate > 0.1) {
            cards[i].style.transform = 'translate3d(0,' + (-nextTranslate) + 'px,0)';
          } else {
        ...
        var isCovered = false;
        if (cards[i + 1]) {
          var nextCardPredictedTop = rects[i + 1].top + appliedTranslates[i + 1] - nextTranslates[i + 1];
    ...
    function applyDepthScale() {
      for (var j = 0; j < last; j++) {
        var currentPredictedBottom = rects[j].bottom + appliedTranslates[j] - nextTranslates[j];
        var nextPredictedTop = rects[j + 1].top + appliedTranslates[j + 1] - nextTranslates[j + 1];
    ...
    // OPTIMIZATION 2: Dedicated handler for layout shifts to rebuild the cache
    function handleLayoutChange() {
      cacheStaticMeasurements();
      requestUpdate();
    }

    // Initialize the cache before the first frame
    cacheStaticMeasurements();
    ...
    if ('ResizeObserver' in window) {
      var resizeObserver = new ResizeObserver(handleLayoutChange);
